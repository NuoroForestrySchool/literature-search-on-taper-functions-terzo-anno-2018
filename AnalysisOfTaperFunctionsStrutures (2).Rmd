...
BibOptions(cite.style = "authoryear", bib.style = "authoryear", no.print.fields = c("url"), check.entries = FALSE)
...
#    xtable(caption = zbib$title[[zid]],
    xtable(align = c("p{0cm}" ,"p{2.5cm}" , "p{11cm}" )) %>%
#    kable("latex", booktabs = T) %>%
#    column_spec(1:2, width = "6cm") %>%
#    kable_styling(latex_options="scale_down") %>%
#    kable_styling(full_width = F) %>%
#    column_spec(1, width = c("2cm")) %>%
    print(comment = F, include.rownames = F, include.colnames =F, caption.placement = 'top')
#    print()
}
```
